...
# Read the info in every function to get the proper understanding of desired output

import json

filepaths = 'C:\Users\vikas\Downloads'
...
def get_oldest(data):
    ...
    for i in data["AVENGERS"]:
        if data["AVENGERS"][i]['age'] > max_:
            max_ = data["AVENGERS"][i]['age']
            oldest = data["AVENGERS"][i].copy()

    for i in data["DC"]:
        if data["DC"][i]['age'] > max_:
            max_ = data["DC"][i]['age']
            oldest = data["DC"][i].copy()

        if data["DC"][i]['age'] == max_:
            oldest[c] = data["DC"][i].copy()
            c += 1
...
def get_oldest_avenger(data):
    global oldest_avenger
    max_ = 0
    for i in data["AVENGERS"]:
        if data["AVENGERS"][i]['age'] > max_:
            max_ = data["AVENGERS"][i]['age']
            oldest_avenger = data["AVENGERS"][i].copy()
